(ns airbyte.main
  "Kotodama WASM entrypoint for the Airbyte clean-room actor (L4) — Clojure port.
  ...
  facts (`airbyte.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "airbyte")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"airbyte.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "airbyte-compat" :tier tier :entities entities} 200])
...
(defn start! [] :airbyte-compat/ready)
